...
object ConfigRebuilder {
    ...
    private fun rebuildVmess(
        ...
        // cf-optimizor passes VMess through untouched (the fm/cs/fp query keys
        // only apply to VLESS/Trojan share links). We only keep the IP swap.
    ...
    // PattNG reads these keys from the share-link query string:
    //   fp = fingerprint (unsafe), cs = cipher suites, fm = FinalMask JSON.
    // This mirrors the cf-optimizor tool exactly (same keys, same encoding).
    private fun applyAntiFilterToQuery(originalQuery: String, af: AntiFilter): String {
        ...
        if (af.fragmentJson.isNotBlank()) {
            params["fm"] = Uri.encode(af.fragmentJson)
        }

        if (af.cipherSuites.isNotBlank()) {
            params["cs"] = Uri.encode(af.cipherSuites)
        }
